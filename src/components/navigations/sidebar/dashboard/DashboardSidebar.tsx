...
export default function DashboardSidebar() {
  ...
  useEffect(() => {
    if (!isStartProjectClicked || !userProfileData) {
      return;
    }
    ...
    router.refresh();
  }, [userProfileData, isStartProjectClicked, router]);
      ...
      >
        <div className={"flex flex-col gap-y-2"}>
          <div className={"flex flex-col gap-y-2 py-2"}>
            <div
              className={
                "cursor-pointer rounded-full border-2 border-light-accent-100 px-4 py-2 hover:opacity-75"
              }
              onClick={() => {
        ...
        <div className={"flex flex-col gap-y-2"}>
          ...
          <div className={"text-sm text-light-text-100"}>
            ...
            center contains a number of articles aimed at getting you started
            with your journey here at Regenera.
          </div>
          <Link
            href={"/"}
            className={
