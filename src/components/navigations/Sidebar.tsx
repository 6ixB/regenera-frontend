...
export default function Sidebar({isSidebarOpen, toggleSidebar}: SidebarProps){
    ...
    return(
        ...
        ${isSidebarOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'} justify-end z-50 transition-opacity ease-in-out duration-300`}>

            <div className={`flex flex-col w-3/5 h-full bg-light-background-100 shadow py-3 px-6 gap-y-4 text-end
            transition-transform duration-200 ${isSidebarOpen ? 'translate-x-0' : 'translate-x-full'} `}>
