...
export default function CreateProjectDetailsTabObjectivesModal({
  ...
  return (
    <div
      ...
      className={cn(
        `size-full fixed bg-black bg-opacity-40 top-0 start-0 z-[80] overflow-x-hidden overflow-y-auto select-none pointer-events-none`,
        className
      )}
    >
      <div
        id="hs-scroll-inside-body-modal"
        className="container ease-out transition-all sm:w-full m-3 sm:mx-auto h-[calc(100%-3.5rem)]"
      >
        <div className="max-h-full overflow-hidden flex flex-col bg-white border shadow-sm rounded-xl pointer-events-auto">
          ...
          <div className="grid grid-cols-3 p-2 gap-2 overflow-y-auto">
            {objectives.map((objective, idx) => (
              <div
                key={idx}
                className="h-[12rem] w-full rounded-md border relative"
              >
                <Image
                  width={0}
                  height={0}
                ...
                />
                <div
                  className="cursor-pointer h-fit w-fit bg-light-background-100 aspect-square border rounded-full absolute top-2 right-2 transition-all hover:bg-light-background-200"
          ...
          <div className="flex justify-end items-center gap-x-2 py-3 px-4 border-t">
            <Button variant={"solid"} className="py-2 px-2">
              <div
                className="flex h-full gap-x-2 items-center"
                onClick={handleAddObjectives}
              >
                <ImagePlus className="h-4 w-auto text-light-background-100" />
                <p className="font-medium text-sm text-light-background-100">
                  Add Photos
                </p>
              </div>
            </Button>
            ...
            <Button variant={"solid"} className="py-2 px-2">
              <p
                className="font-medium text-sm text-light-background-100"
                onClick={handleOpenModel}
              >
                Close
              </p>
